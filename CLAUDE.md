...
- **Animation.** `Reveal` (`src/components/ui/reveal.tsx`) is currently a pass-through wrapper with no animation, and `motion` is not a declared dependency in `package.json` despite mentions in `README.md`/`AGENTS.md`. Any animation added must respect `prefers-reduced-motion`.
