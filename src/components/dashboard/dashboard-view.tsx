...
export function DashboardView({ data, isAdmin = false }: { data: DashboardData; isAdmin?: boolean }) {
  ...
  return <AppShell isAdmin={isAdmin} isDemo={data.isDemo}>
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
      ...
      <Reveal delay={0.06}><Card className="h-full hover:-translate-y-0.5"><CardHeader><CardTitle>Net earnings history</CardTitle><CardDescription>Earned net pay by week.</CardDescription></CardHeader><CardContent className="h-60"><ResponsiveContainer width="100%" height="100%"><BarChart data={data.earningsHistory} margin={{ left: -15, right: 4, top: 4 }}><CartesianGrid vertical={false} stroke="var(--border)" /><XAxis dataKey="label" tickLine={false} axisLine={false} tick={{ fill: "var(--muted-foreground)", fontSize: 12 }} /><YAxis tickLine={false} axisLine={false} tick={{ fill: "var(--muted-foreground)", fontSize: 12 }} tickFormatter={(value) => `$${Math.round(Number(value) / 100)}`} /><Tooltip cursor={{ fill: "var(--surface-subtle)" }} formatter={(value) => formatCents(Number(value))} contentStyle={chartTooltip} itemStyle={{ color: "var(--foreground)", fontSize: "14px" }} labelStyle={{ color: "var(--muted-foreground)", fontWeight: "500", marginBottom: "4px" }} /><Bar dataKey="netCents" fill="var(--success)" radius={[7, 7, 2, 2]} animationDuration={650} /></BarChart></ResponsiveContainer></CardContent></Card></Reveal>
    </section>

    <section className="mt-6 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
      <Reveal><Card className="h-full hover:-translate-y-0.5"><CardHeader><CardTitle>Weekly history</CardTitle><CardDescription>Logged hours over the last five weeks</CardDescription></CardHeader><CardContent className="h-72"><ResponsiveContainer width="100%" height="100%"><BarChart data={data.history} margin={{ left: -20, right: 8, top: 4 }}><CartesianGrid vertical={false} stroke="var(--border)" /><XAxis dataKey="label" tickLine={false} axisLine={false} tick={{ fill: "var(--muted-foreground)", fontSize: 12 }} /><YAxis tickLine={false} axisLine={false} tick={{ fill: "var(--muted-foreground)", fontSize: 12 }} tickFormatter={(value) => `${Math.round(value / 60)}h`} /><Tooltip cursor={{ fill: "var(--surface-subtle)" }} formatter={(value) => `${formatHours(Number(value))}h`} contentStyle={chartTooltip} itemStyle={{ color: "var(--foreground)", fontSize: "14px" }} labelStyle={{ color: "var(--muted-foreground)", fontWeight: "500", marginBottom: "4px" }} /><Bar dataKey="loggedMinutes" fill="var(--primary)" radius={[7, 7, 2, 2]} animationDuration={700} /></BarChart></ResponsiveContainer></CardContent></Card></Reveal>
