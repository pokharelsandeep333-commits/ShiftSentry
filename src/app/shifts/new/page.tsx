...
export default async function NewShiftPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [{ data: jobs }, params] = await Promise.all([
    supabase.from("jobs").select("id,name,color").eq("user_id", profile.id).is("archived_at", null).order("name"),
    searchParams,
  ]);
  const initialShift = prefillFromQuery(params, jobs ?? []);
  ...
  return <AppShell isAdmin={profile.role === "ADMIN"} userEmail={profile.email}>
    ...
    <Card className="max-w-2xl">
      ...
      <CardContent>{jobs?.length ? <ShiftForm mode="create" jobs={jobs} initialShift={initialShift} /> : <div className="space-y-4 rounded-2xl border border-dashed p-5"><p className="text-sm leading-6 text-[var(--muted-foreground)]">Create a job before adding a shift.</p><Link href="/jobs"><Button>Create a job</Button></Link></div>}</CardContent>
    </Card>
  </AppShell>;
}
